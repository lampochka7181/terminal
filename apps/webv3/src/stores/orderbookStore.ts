...
export const useOrderbookStore = create<OrderbookState>()(
  subscribeWithSelector((set, get) => ({
    ...
    setOrderbook: (outcome, bidsData, asksData, sequenceId) => {
      ...
      // Process YES orderbook - allow clearing when MM sends empty data
      // This ensures orderbook properly reflects when MM stops quoting one/both sides
      const yesBids = processLevels(bidsData, 'bid', currentBook.bids);
      const yesAsks = processLevels(asksData, 'ask', currentBook.asks);
