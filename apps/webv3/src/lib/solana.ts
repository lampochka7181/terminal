...
export const PROGRAM_ID = new PublicKey(
  import.meta.env.VITE_PROGRAM_ID || '5Kq43SR2HUNsyNZWaau1p8kQzAvW2UA2mAvempdchTrk'
);
...
/**
 * Truncate a number to 6 decimal places to match on-chain precision (SHARE_MULTIPLIER = 10^6)
 * Uses floor to ensure we never try to sell more shares than owned
 */
export function truncateToOnChainPrecision(n: number): number {
 ...
 */
export function validateSize(size: number): { valid: boolean; error?: string; truncatedSize?: number } {
  // Auto-truncate to 6 decimals to match on-chain precision
  const truncatedSize = truncateToOnChainPrecision(size);
  
  if (truncatedSize < 0.001) {
    return { valid: false, error: 'Size must be at least 0.001 contracts' };
  }
  if (truncatedSize > 100_000) {
    return { valid: false, error: 'Size must be at most 100,000 contracts' };
  }
  return { valid: true, truncatedSize };
}
